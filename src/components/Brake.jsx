...
export default function Brake({ data }) {
  const { setRating } = useStore();
  const brakeParts = [
    { key: "frontBrakePads", label: "Front Brake Pads" },
    { key: "rearBrakePads", label: "Rear Brake Pads" },
    { key: "frontBrakeRotors", label: "Front Brake Rotors" },
    { key: "rearBrakeRotors", label: "Rear Brake Rotors" },
    { key: "brakePedal", label: "Brake Pedal" },
    { key: "handbrake", label: "Handbrake / Parking Brake" },
  ];

  const avgRating = useMemo(() => {
    const ratings = brakeParts.map((part) => data?.[part.key]?.rating);
    ...
    <div id="brake">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Brakes</h4>
