...
export default function Chassis({ data }) {
  const { setRating } = useStore();
  const chassisParts = [
    { key: "aPillar", label: "A-Pillar" },
    { key: "bPillar", label: "B-Pillar" },
    { key: "cPillar", label: "C-Pillar" },
    { key: "rearChassisRails", label: "Rear Chassis Rails" },
    { key: "frontChassisRails", label: "Front Chassis Rails" },
    { key: "crossMembers", label: "Cross Members" },
  ];

  const avgRating = useMemo(() => {
    const ratings = chassisParts.map((part) => data?.[part.key]?.rating);
    ...
    <div id="chassis">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Chassis & Body</h4>
