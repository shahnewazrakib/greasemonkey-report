...
export default function Gearbox({ data }) {
  const { setRating } = useStore();
  const gearboxParts = [
    { key: "gearSelector", label: "Gear Selector" },
    { key: "clutch", label: "Clutch" },
    { key: "transmissionFluid", label: "Transmission Fluid" },
    { key: "shiftQuality", label: "Shift Quality" },
  ];

  const avgRating = useMemo(() => {
    const ratings = gearboxParts
      .filter((part) => {
        const partData = data?.[part.key];
        if (part.key === "clutch" && !partData?.isAvailable) return false;
        return true;
      })
    ...
    <div id="gearbox">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Transmission</h4>
          ...
          {gearboxParts.map((part) => {
            ...
            return (
              <div key={part.key} className={styles.row}>
                ...
                {part.key === "clutch" && !partData?.isAvailable ? (
                  <>
                    <div>N/A</div>
