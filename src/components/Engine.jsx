...
export default function Engine({ data }) {
  ...
  const engineParts = [
    ...
    { key: "starterMotor", label: "Starter Motor" },
    { key: "waterPump", label: "Water Pump" },
    { key: "oilSump", label: "Oil Sump" },
    { key: "driveBelt", label: "Drive Belt" },
    { key: "radiator", label: "Radiator" },
    { key: "engineMount", label: "Engine Mount" },
    { key: "sparkPlug", label: "Spark Plugs" },
    { key: "injector", label: "Injectors" },
    { key: "coils", label: "Coils" },
  ];

  const avgRating = useMemo(() => {
    const ratings = engineParts
      .filter((part) => {
        const partData = data?.[part.key];
        if (part.key === "sparkPlug" && !partData?.isAvailable) return false;
        return true;
      })
    ...
    <div id="engine">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Engine</h4>
          ...
          {engineParts.map((part) => {
            ...
            return (
              <div key={part.key} className={styles.row}>
                ...
                {part.key === "sparkPlug" && !partData?.isAvailable ? (
                  <>
                    <div>N/A</div>
