...
export default function Differential({ data }) {
  const { setRating } = useStore();
  const differentialParts = [
    { key: "differentialBushes", label: "Differential Bushes" },
    { key: "driveShaft", label: "Drive Shaft" },
  ];

  const avgRating = useMemo(() => {
    const ratings = differentialParts.map((part) => data?.[part.key]?.rating);
  ...
  useEffect(() => {
    if (avgRating) {
      setRating("differential", avgRating);
    }
  }, [avgRating]);
    ...
    <div id="differential">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Differential</h4>
