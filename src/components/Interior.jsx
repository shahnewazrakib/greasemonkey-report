...
export default function Interior({ data }) {
  ...
  const interiorParts = [
    { key: "seatsDriver", label: "Seats (Driver)" },
    { key: "seatsPassenger", label: "Seats (Passenger)" },
    { key: "seatsRear", label: "Seats (Rear)" },
    { key: "doorsDriverSide", label: "Doors (Driver Side)" },
    { key: "doorsFrontLeft", label: "Doors (Front Left)" },
    { key: "doorsRearLeft", label: "Doors (Rear Left)" },
    { key: "doorsRearRight", label: "Doors (Rear Right)" },
    { key: "dashboard", label: "Dashboard" },
    { key: "steeringWheel", label: "Steering Wheel" },
    { key: "speedOdometer", label: "Speed Odometer" },
    { key: "airConditioning", label: "Air Conditioning" },
    { key: "heater", label: "Heater" },
  ];

  const avgRating = useMemo(() => {
    const ratings = interiorParts.map((part) => data?.[part.key]?.rating);
    ...
    <div id="interior">
      <div className="rounded-xl border">
        <div className="bg-muted rounded-t-xl py-3 px-4 border-b flex items-center justify-between gap-2">
          <h4 className="font-semibold sm:text-lg">Interior</h4>
